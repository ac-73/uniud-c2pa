...
public class VerifyAssertionsCommandHandler {
	...
	if (c2paStore != null) {
		...
		try {
		    ...
		    result.addAll(c2paVerify.verifyAssertions(c2paManifest, bais));
		} finally {
		    try {
			bais.close();
		    } catch (IOException e) {
	    ...
	    if (result.isEmpty()) {
		MessageDialog.openInformation(shell, "Verifica asserzioni", "Le asserzioni risultano valide!");
	    } else {
		...
		MessageDialog.openError(shell, "Verifica asserzioni", stringBuffer.toString());
	    }
	}
    }
}
